 ...
 *
 * Flow:
 *   1. Resolve compound name → CID via PubChem PUG REST
 *   2. Fetch 3D conformer coords + atom elements
 *   3. Centre + scale to fit in ~22³ voxel grid
 *   4. Map element → CPK colour
 *   5. Return voxelised blocks + metadata
 */
...
// ─── CPK colour map (element symbol → hex) ───────────────────────────────────
...
// ─── PubChem 3D JSON type (simplified) ───────────────────────────────────────
...
// ─── Deduplicate voxels (last writer wins keeps most recent colour) ───────────
function dedup(blocks: { x:number; y:number; z:number; color:string; label:string }[]) {
...
export async function POST(req: Request) {
  try {
    ...
    if (!name?.trim()) return NextResponse.json({ error: 'Compound name required' }, { status: 400 });
    ...
    if (!cidRes.ok) return NextResponse.json({ error: `Compound "${name}" not found in PubChem` }, { status: 404 });
    const cidJson = await cidRes.json();
    const cid: number = cidJson.IdentifierList?.CID?.[0];
    if (!cid) return NextResponse.json({ error: 'Could not resolve CID' }, { status: 404 });
    ...
    if (!mol3dRes.ok) {
      // Fall back to 2D (no Z coords, still useful)
    ...
    // Also fetch compound title / formula from properties
